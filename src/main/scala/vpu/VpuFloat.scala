...
/** Synthesizable FP32 EXP approximation.
  ...
  * The datapath is eight cycles and fully pipelined. Subnormal outputs are
...
object VpuExpApprox {
  /** Issue-to-result latency. The pipeline remains initiation-interval one. */
  val Latency = 8
}
...
class VpuExpApprox extends Module {
  ...
  // Q39.24.  The explicit register here is the first of the eight externally
  ...
  // Stage 3: 64-entry 2^(i/64) lookup. Register the selected table value and
  // residual before any polynomial multiplication so the ROM mux is not in a
  // multiplier's input path.
  ...
  val tableValueS3 = RegEnable(expTable(tableIndex), validS2)
  val remainderQ24S3 = RegEnable(remainderQ24, validS2)

  // Stage 4: calculate the linear term and r^2 in parallel. There is one
  // multiplier level on either path, rather than a serial r^2/coefficient
  // chain.
  val linear = (remainderQ24S3 * ln2Q31.U) >> 24
  val remSquared = remainderQ24S3 * remainderQ24S3

  val validS4 = RegNext(validS3, false.B)
  val metadataS4 = RegEnable(metadataS3, validS3)
  val nS4 = RegEnable(nS3, validS3)
  val tableValueS4 = RegEnable(tableValueS3, validS3)
  val linearS4 = RegEnable(linear, validS3)
  val remSquaredS4 = RegEnable(remSquared, validS3)

  // Stage 5: calculate the quadratic term and form the local second-order
  // factor. This stage has one multiplier followed by a short adder tree.
  val quadratic = (remSquaredS4 * ln2SquaredHalfQ31.U) >> 48
  val localFactorQ31 = (BigInt(1) << 31).U(34.W) +& linearS4 +& quadratic

  val validS5 = RegNext(validS4, false.B)
  val metadataS5 = RegEnable(metadataS4, validS4)
  val nS5 = RegEnable(nS4, validS4)
  val tableValueS5 = RegEnable(tableValueS4, validS4)
  val localFactorQ31S5 = RegEnable(localFactorQ31, validS4)

  // Stage 6: multiply the table value by the local interpolation factor. This
  // multiplier now has registered operands and a registered result.
  val approximationQ31 = (tableValueS5 * localFactorQ31S5) >> 31

  val validS6 = RegNext(validS5, false.B)
  val metadataS6 = RegEnable(metadataS5, validS5)
  val nS6 = RegEnable(nS5, validS5)
  val approximationQ31S6 = RegEnable(approximationQ31, validS5)

  // Stage 7: normalize and round to nearest-even.  The same stage performs the
  // second rounding needed when the result falls into the FP32 subnormal range.
  val significandBeforeRound = approximationQ31S6(31, 8)
  val roundBit = approximationQ31S6(7)
  val sticky = approximationQ31S6(6, 0).orR
  ...
  val resultExponentS = nS6 + 127.S + significandCarry.asUInt.zext
  ...
  val validS7 = RegNext(validS6, false.B)
  val metadataS7 = RegEnable(metadataS6, validS6)
  val resultExponentS7 = RegEnable(resultExponentS, validS6)
  val normalResultS7 = RegEnable(normalResult, validS6)
  val subnormalResultS7 = RegEnable(subnormalResult, validS6)
  val exponentOverflowS7 = RegEnable(exponentOverflow, validS6)
  val exponentSubnormalS7 = RegEnable(exponentSubnormal, validS6)

  // Stage 8: select architectural special-case results and flags.  Keeping
  ...
  val rawResult = Mux(metadataS7.isNaN, VpuFloat.canonicalNaN.U(32.W),
    Mux(metadataS7.isInf && !metadataS7.sign,
      VpuFloat.positiveInfinity.U(32.W),
      Mux(metadataS7.isInf && metadataS7.sign, 0.U,
        Mux(metadataS7.finiteOverflow || exponentOverflowS7,
          VpuFloat.positiveInfinity.U(32.W),
          Mux(metadataS7.finiteUnderflow || resultExponentS7 < -23.S, 0.U,
            Mux(exponentSubnormalS7, subnormalResultS7, normalResultS7))))))
  ...
  val flags = Mux(metadataS7.isNaN,
    Mux(metadataS7.isSignalingNaN, "b10000".U(5.W), 0.U(5.W)),
    Mux(metadataS7.isInf, 0.U(5.W),
      Mux(metadataS7.finiteOverflow || exponentOverflowS7,
        "b00101".U(5.W),
        Mux(metadataS7.finiteUnderflow || exponentSubnormalS7,
          "b00011".U(5.W),
          Mux(metadataS7.finiteNonZero, "b00001".U(5.W), 0.U(5.W))))))

  val result = Wire(new VpuFpResult)
  result.data := rawResult
  result.fflags := flags
  val validS8 = RegNext(validS7, false.B)
  val resultS8 = RegEnable(result, validS7)
  io.out.valid := validS8
  io.out.bits := resultS8
}
